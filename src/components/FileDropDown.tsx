...
import { decryptBlob } from "@/lib/cryptoModule";
...
function FileDropDown({
  ...
  const openFile = async (fileLink: string) => {
    const res = await fetch(fileLink);
    const buf = await res.arrayBuffer();
    // fetch meta we saved in Firebase Storage metadata
    const meta = (await fetch(fileLink + "?alt=media&metadata=true")).headers; // quick hack
    const kmsRes = await fetch("/api/kms/unwrap/" + file.fileName, {
      headers: { Authorization: `Bearer ${localStorage.jwt}` },
    });
    const { wrappedKey } = await kmsRes.json();
    const plaintextBlob = await decryptBlob(buf, { ...meta, wrappedKey });
    const o = URL.createObjectURL(plaintextBlob);
